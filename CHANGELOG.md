...
- Synced skills with `developer-hub` up to commit `d466ebf4` — **FCC**: TEE extension ID discovery now uses `nextPublicExtensionId()` instead of `extensionsCounter()` (public extension IDs start at `0x10000`); `post-build.sh` runs a new `set-governance` onchain step (`GOVERNANCE_SIGNERS`/`GOVERNANCE_THRESHOLD`, `InvalidGovernanceHash` error); `WeatherInsurance.sol` TEE signature verification is now domain-separated (`keccak256(abi.encode(TEE_ACTION_RESULT_PREFIX, chainId, resultHash))` instead of the raw result hash); linked the `fce-weather-insurance-x402-agent` AI agent showcase. **Smart Accounts**: added the Control USDT0 guide — fee-only `0xFE` custom instructions with `netMintAmountXrp: 0` to transfer/swap ERC-20 balances (e.g. USDT0) held by a personal account without minting FXRP.
